...
print("\033[1;34m             ███╔╝    ╚██╔╝     ██║   ██╔══██║")
print("\033[1;34m            ███████╗   ██║      ██║   ██║  ██║")
print("\033[1;34m            ╚══════╝   ╚═╝      ╚═╝   ╚═╝  ╚═╝ Auto TikTOk Zefoy v2")
print()
print("    " + "⏦" * 51)
print(" Nguyễn Ngọc Thành                  " + "DevOps: Lê Trọng Truyền\n\n")
class Zefoy:
    def __init__(self):
        ...
        url1=input("\033[1;37m[" +"ZYTH] ==>  Nhập link video: ")
        self.url=url1
    ...
    def find_video(self):
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                print('[\033[1;33mZYTH] ==>  Phiên hết hạn. Đang đăng nhập lại...')
                self.send_captcha()
                return
            elif 'service is currently not working' in self.video_info:
                return (True, '[\033[1;33mZYTH] ==>  Dịch vụ hiện không hoạt động, hãy thử lại sau.')
                ...
                try: 
                    ...
                    zyth = int(re.findall(r'ltm=(\d*);', self.video_info)[0])
                except: 
                    return (False,)
                if zyth==0:self.find_video()
                elif zyth >= 1000:
                    print('[\033[1;33mZYTH] ==>  IP của bạn đã bị FBI để ý, hãy cẩn trọng ( delay >= 1000)')
                _=time.time()
                while time.time()-2<_+zyth:
                    t-=1
                    print('                                                   ', end = '\r')
    ...
    def use_service(self):
        ...
        if 'Session expired. Please re-login' in res:
            ...
            return ('[\033[1;33mZYTH] ==>  Dịch vụ hiện không hoạt động, hãy thử lại sau.')
        else:
    ...
    def get_video_id(self, url_ = None, set_url=True):
        ...
        if request.text == '':
            print('[\033[1;33mZYTH] ==>  Link video không hợp lệ')
            return False
        else:
            json_ = request.json()
    ...
    def update_name(self):
        while True:
            try:
                ...
                self.text = f"[ZYTH] ==>  | Views: {video_info['viewCount']} "
            except:
                pass
            time.sleep(5)
    ...
    def select_service(self):
        while True:
            ...
            self.get_table()
            print(f"{xanh_la}[ZYTH] ==>  Nhập dịch vụ bạn muốn sử dụng (nhập ID):", end=' ')
            service_id = input()
            if service_id.isdigit():
                ...
                if service_id in range(1, len(self.services) + 1):
                    ...
                    print(f"{do}[\033[1;33mZYTH] ==>  ID dịch vụ không hợp lệ. Vui lòng nhập lại.")
            else:
                print(f"{do}[\033[1;33mZYTH] ==>  ID dịch vụ không hợp lệ. Vui lòng nhập lại.")
        ...
        while True:
            try:
                if 'Service is currently not working, try again later' in str(self.use_service()):
                    print(f'{do}[\033[1;33mZYTH] ==> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
                    time.sleep(5)
            except Exception as e:
                print(f'LỖI NGHIÊM TÚC | thử lại sau 10 giây......|| {e}')
                time.sleep(10)
