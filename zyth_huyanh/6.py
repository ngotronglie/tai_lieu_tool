...
class Zefoy:
	...
	def __init__(self):
		...
		self.text = '\033[1;33mZYTH'

		url1=input("[\033[1;33mZYTH] ==>  Nhập link video: ")
	...
	def get_captcha(self):
		...
		try:
			...
			print('[\033[1;33mZYTH] ==>  Đang giải capcha..')

			return False

		except Exception as e:

			print(f"[\033[1;33mZYTH] ==>  Không thể giải captcha: {e}")
	...
	def send_captcha(self, new_session = False):
		...
		if self.get_captcha(): print('[\033[1;33mZYTH] ==>  Đang kêt nối đến session');return (True, 'The session already exists')
		...
		if 'Enter Video URL' in request.text: 
			...
			print('[\033[1;33mZYTH] ==>  Session đã được tạo')

			open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))

			print(f"[\033[1;33mZYTH] ==>  Giải capcha thành công: {captcha_solve}")
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[\033[1;33mZYTH] ==>  Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return

			elif 'service is currently not working' in self.video_info: return (True,'[\033[1;33mZYTH] ==>  Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
				...
				elif lamtilo >= 1000: print('[\033[1;33mZYTH] ==>  Your IP was banned')

				# print(f'[\033[1;33mZYTH] ==>  Thời gian chờ để tiếp tục tăng: {t} giây')

				# for lam in range(t,0,-1):

				# 	lam-=1

				# 	print(f"[\033[1;33mZYTH] ==>  Vui lòng chờ: {lam}",end="\r")
				...
				while time.time()-2<_+lamtilo:
					...
					print("[\033[1;33mZYTH] ==>  Vui lòng chờ: {0} ".format(t)+"giây .", end="\r")
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: print('[\033[1;33mZYTH] ==>  Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return ""

		elif 'Too many requests. Please slow' in res: time.sleep(3)

		elif 'service is currently not working' in res: return ('[\033[1;33mZYTH] ==>  Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[\033[1;33mZYTH] ==>  Link video không hợ lệ'); return False
	...
	def update_name(self):
		...
		while True:
			...
			try:
				...
				self.text = f"[\033[1;33mZYTH] ==>  | Views: {video_info['viewCount']} "
...
#     print(f"{trang}[\033[1;33mZYTH] ==>  Link key: {link1}")
...
# nhap = input(f"{trang}[\033[1;33mZYTH] ==>  Nhập key vừa lấy được: ")

# if nhap == key:

if 1:

    print(f"{xanh_la}[\033[1;33mZYTH] ==>  Key đúng mời bạn vô tool")
    ...
    print(f"{do}[\033[1;33mZYTH] ==>  Key sai vui lòng vượt lại")
...
while True:
	...
	try: 
		...
		if 'Service is currently not working, try again later' in str(Z.use_service()): print(f'{do}[\033[1;33mZYTH] ==> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.');time.sleep(5)
