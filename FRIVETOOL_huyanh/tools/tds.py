...
print(f"{red}├─────┼────────────────────────────────────┼─────────┼─────────┤")
print(f"{red}│ {vang} 6 {red} │{lam} QUAY LẠI        {red}                   │{tim}   =.=   {red}│ {lam}  NEXT {red} │")
print(f"{red}└─────┴────────────────────────────────────┴─────────┴─────────┘\n")
chon = input(f"{thanh_dep}{luc}Nhập Lựa Chọn{trang}: ")
os.system("cls" if os.name == "nt" else "clear")
try:
        if chon == '1':
                run = requests.get(f'https://raw.githubusercontent.com/FRIVE-TOOL/FRIVE-TOOL/main/facebook.py').text
        elif chon == '2':
                run = requests.get(f'https://raw.githubusercontent.com/FRIVE-TOOL/FRIVE-TOOL/main/tdspro5.py').text
        elif chon == '3':
                run = requests.get(f'https://raw.githubusercontent.com/FRIVE-TOOL/FRIVE-TOOL/main/ins.py').text
        elif chon == '4':
                run = requests.get(f'https://raw.githubusercontent.com/FRIVE-TOOL/FRIVE-TOOL/main/tiktok.py').text
        elif chon == '5':
                run = requests.get(f'https://raw.githubusercontent.com/FRIVE-TOOL/FRIVE-TOOL/main/tiktok_speed.py').text
        elif chon == '6':
